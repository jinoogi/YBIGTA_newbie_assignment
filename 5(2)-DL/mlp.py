...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X, training=True, dropout_rate=0.0):
        """
        ...
        h1 = X @ W1 + b1          # 첫 번째 hidden layer
        z1 = relu(h1)             # ReLU activation
        z1, dropout_mask1 = dropout(z1, dropout_rate, training)  # Dropout

        h2 = z1 @ W2 + b2         # 두 번째 hidden layer
        z2 = leakyrelu(h2)        # LeakyReLU activation
        z2, dropout_mask2 = dropout(z2, dropout_rate, training)  # Dropout

        h3 = z2 @ W3 + b3         # 세 번째 hidden layer
        z3 = tanh(h3 + h1)        # tanh with skip connection (H3 + H1)
        z3, dropout_mask3 = dropout(z3, dropout_rate, training)  # Dropout

        h4 = z3 @ W4 + b4         # output layer
        ...
        cache = {'h1': h1, 'z1': z1, 'h2': h2, 'z2': z2, 'h3': h3, 'z3': z3, 'h4': h4, 'y_hat': y_hat,
                 'dropout_mask1': dropout_mask1, 'dropout_mask2': dropout_mask2, 'dropout_mask3': dropout_mask3}
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        dropout_mask1, dropout_mask2, dropout_mask3 = cache['dropout_mask1'], cache['dropout_mask2'], cache['dropout_mask3']
        ...
        # Layer 3 gradients (with skip connection)
        # z3 = tanh(h3 + h1)이므로 dz3/dh3 = (1-z3^2), dz3/dh1 = (1-z3^2)
        dh3_tanh = dz3 * (1 - z3**2)  # tanh derivative
        dh3 = dh3_tanh                 # h3에서 오는 gradient
        dh1_skip = dh3_tanh            # skip connection에서 h1으로 오는 gradient
        
        # Apply dropout mask to gradients
        if dropout_mask3 is not None:
            dh3 = dh3 * dropout_mask3
            dh1_skip = dh1_skip * dropout_mask3
        ...
        # Layer 2 gradients  
        dh2 = dz2 * (h2 > 0) + 0.01 * (h2 <= 0)  # leaky relu derivative
        
        # Apply dropout mask
        if dropout_mask2 is not None:
            dh2 = dh2 * dropout_mask2
            
        db2 = np.sum(dh2, axis=0)
        dW2 = z1.T @ dh2 + 2 * L2_norm * W2
        dz1 = dh2 @ W2.T
        
        # Layer 1 gradients (combine direct and skip gradients)
        dh1_direct = dz1 * (h1 > 0)   # relu derivative
        dh1 = dh1_direct + dh1_skip   # combine gradients from direct path and skip connection
        
        # Apply dropout mask
        if dropout_mask1 is not None:
            dh1 = dh1 * dropout_mask1
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, learning_rate=1e-3, L2_norm=0.0, dropout_rate=0.0, epoch=20000, print_loss=True):
        """
        Descriptions:
        ...
        for it in range(epoch):
            ...
            y_hat, cache = self.forward_propagation(X_train, training=True, dropout_rate=dropout_rate)
            ...
            for key in ['dW1', 'db1', 'dW2', 'db2', 'dW3', 'db3', 'dW4', 'db4']:
                grad[key] = np.clip(grad[key], -max_grad, max_grad)
    ...
    def predict(self, X):
        ...
        y_hat, _ = self.forward_propagation(X, training=False, dropout_rate=0.0)  # No dropout during inference
...
def dropout(x, dropout_rate=0.5, training=True):
    ...
    Args:
        x: Input array
        dropout_rate: Probability of dropping a unit (0.0 to 1.0)
        training: Whether in training mode
    
    Returns:
        output: Output after dropout
